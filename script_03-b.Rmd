...
sep_dif  <- inner_join(sep_old, sep_new, by = "id", suffix = c("_old", "")) %>%
...
```{r sc1b}
sep_dif1  <- inner_join(sep_old, sep_new, by = "id", suffix = c("_old", "")) %>%
  mutate(enrollment_diff = u_grad - u_grad_old) %>%
  select(id, title, name, department, u_grad_old, u_grad, enrollment_diff) %>% 
  drop_na() %>% 
...
sep_dif1 %>%
  gt() +
  tab_header(title = "Course Enrollment Old") +
  cols_label(id = "ID", title = "Title", name = "Name", department = "Department", u_grad_old = "")

sep_dif_total <- full_join(sep_dif, sep_dif1) %>%
  ggplot(aes(enrollment_diff, name)) +
